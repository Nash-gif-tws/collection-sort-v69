// app/routes/api.analytics.color-curve.ts
...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  const rows = await prisma.$queryRaw<{ color: string | null; units: number }[]>`
    SELECT COALESCE(v."color",'Unknown') AS color, SUM(ol."qty")::int AS units
    FROM "OrderLine" ol
